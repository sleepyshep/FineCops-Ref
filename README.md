...
🔥 **[2024/06/17]** We released the **[FineCops-Ref dataset](https://github.com/liujunzhuo/FineCops-Ref)** — featuring controlled difficulty levels and challenging negative samples.
...
7. **Focus-Enhancement Strategy (Optional)**
   
    To implement the focus-enhancement strategy, simply set the `--focus_enhancement` argument.
...
3. **Instruction Tuning for Multi-Choice Selection**
   
    We fine-tune MLLMs with a small subset of RefCOCO series to improve their instruction-following capability, without risking information leakage. The fine-tuning datasets are:

   - `refcoco_CRS_pos.json` (positive samples)
...
7. **Evaluation**
    ...
    Evaluate model performance on positive and negative samples using:
